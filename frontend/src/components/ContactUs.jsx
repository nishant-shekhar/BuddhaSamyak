...
const ContactUs = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Basic guard (you already have required attributes on inputs)
      const { name, email, subject, message, phone } = formData;

      // Push to /ContactMessages
      const listRef = ref(db, "BuddhaSamyak/ContactMessages");
      await push(listRef, {
        name,
        email,
        phone: phone || "",
        subject,
        message,
        status: "NEW",                 // for your admin panel filters later
        createdAt: serverTimestamp(),  // server time
        userAgent: navigator.userAgent // optional, helpful for diagnostics
      });
      ...
      // auto-hide success UI after a few seconds (optional)
  ...
  const contactInfo = [
    { icon: Phone, title: t("phoneNumber"),       value: t("phoneValue"),  subtitle: t("phoneSubtitle") },
    { icon: Mail,  title: t("emailAddress"),      value: t("emailValue"),  subtitle: t("emailSubtitle") },
    { icon: MapPin,title: t("visitUs"),           value: t("visitValue"),  subtitle: t("visitSubtitle") },
    { icon: Clock, title: t("openingHours_contact"), value: t("hoursValue"), subtitle: t("hoursSubtitle") },
  ];
          ...
          <div className="bg-black rounded-3xl p-8 lg:p-10 border border-gray-800">
            ...
            {submitStatus === "success" ? (
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {/* Error banner (if any) */}
                {submitStatus === "error" && (
                  <div className="rounded-xl border border-red-600 bg-red-500/10 text-red-200 px-4 py-3">
                    {errorMsg}
                  </div>
                )}
